...
if (isset($_POST['login'])) {
    ...
    if ($user) {
        // Verificar se a senha é igual a "aluno@123"
        if ($user['senha'] === 'aluno@123') {
            // Redirecionar para alterar_senha.php
            header("Location: alterar_senha.php?cpf=$cpf");
            exit();
        } else {
            // Senha não é "aluno@123", verificar a senha fornecida
            if ($user['senha'] === $senha) {
                ...
                } elseif ($user['nivel_acesso'] === 'gerente') {
                    header("Location: /refeitorio/acesso/gerente/dashboard_gerente.php");
                } elseif ($user['nivel_acesso'] === 'cliente') {
                    header("Location: /refeitorio/acesso/cliente/dashboard_cliente.php");
                }
                exit();
            } else {
